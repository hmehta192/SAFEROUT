  ...
  const show = useCallback(
    (m: string, t: ToastType = "info") => {
      ...
      timer.current = setTimeout(() => {
        Animated.timing(opacity, { toValue: 0, duration: 250, useNativeDriver: useNative }).start(() =>
          setVisible(false)
        );
      }, 2800);
    },
...
const styles = StyleSheet.create({
  wrap: { position: "absolute", left: spacing.lg, right: spacing.lg, zIndex: 9999, alignItems: "center", pointerEvents: "none" },
  toast: {
    flexDirection: "row",
    alignItems: "center",
    gap: spacing.sm,
